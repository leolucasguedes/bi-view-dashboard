...
interface DistributionCityChartProps {
    users: UserData[];
}

export const DistributionCityChart: React.FC<DistributionCityChartProps> = ({
    users,
}) => {
    const cityCounts: { [key: string]: number } = users.reduce((acc, user) => {
        if (user.cidade) {
            acc[user.cidade] = (acc[user.cidade] || 0) + 1;
        }
        return acc;
    ...
    const topCities = sortedCities.slice(0, 23);
    const counts = topCities.map(city => cityCounts[city]);
